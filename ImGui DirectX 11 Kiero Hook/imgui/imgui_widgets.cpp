...
bool ImGui::Checkbox(const char* label, bool* v) {
    ...
    // Сглаживаем переход для активного состояния
    value->second = ImLerp(value->second, (*v ? 1.f : 0.f), 0.08f);

    // Отображаем текст лейбла
    ...
    // Рисуем границу чекбокса
    draw->AddRect(frame_bb.Min, frame_bb.Max, gui.border.to_im_color(), 10);

    // Рисуем круглый переключатель, который будет двигаться
